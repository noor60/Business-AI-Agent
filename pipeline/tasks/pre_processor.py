...
def data_pre_processing():
    ...
    df_exploded["item_code"] = df_exploded["parsed_items"].apply(
        lambda x: x["item_code"]
    )
    df_exploded["item_name"] = df_exploded["parsed_items"].apply(
        lambda x: x["item_name"]
    )
    df_exploded["item_price"] = df_exploded["parsed_items"].apply(
        lambda x: x["item_price"]
    )
    df_exploded["is_discount"] = df_exploded["parsed_items"].apply(
        lambda x: x["is_discount"]
    )
    df_exploded["total_price"] = df_exploded["parsed_items"].apply(
        lambda x: x["total_price"]
    )
    df_exploded["item_description"] = df_exploded["parsed_items"].apply(
        lambda x: x["item_description"]
    )

    df_exploded[
        ["customer_name", "payment_method", "item_name", "item_description"]
    ] = df_exploded[
        ["customer_name", "payment_method", "item_name", "item_description"]
    ].apply(lambda col: col.astype(str).str.lower())
    ...
    return customer_data, df_exploded


def data_processer():
    customer_path = (
        Path(__file__).resolve().parents[1] / "outputs" / "customers_data_processed.csv"
    )
    order_path = (
        Path(__file__).resolve().parents[1] / "outputs" / "orders_data_processed.csv"
    )

    if customer_path.exists() and order_path.exists():
        processed_customer_data = pd.read_csv(customer_path)
        processed_orders_data = pd.read_csv(order_path)
        print("Data files already exist. Skipping process.")
        return processed_customer_data, processed_orders_data
    else:
        print("Data files do not exist. Fetching and processing data.")
        processed_customer_data, processed_orders_data = data_pre_processing()
        return processed_customer_data, processed_orders_data
